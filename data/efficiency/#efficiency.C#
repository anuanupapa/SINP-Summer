...
void efficiency::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
       ...
       for (int a=0; a<nEle; ++a)
	 {
       ...
       if(c!=2) continue;
       Int_t p;
       if(mag>120 || mag<60) continue;
       
       if(fabs(eleSCEta->at(probe))<1.479 && eleSigmaIEtaIEtaFull5x5->at(probe)<0.018)
	 {
	   p=2;
	   passing_probe = passing_probe + 1;
	 }
       else if(1.479 < abs(eleSCEta->at(probe)) && abs(eleSCEta->at(probe))< 2.4 && eleSigmaIEtaIEtaFull5x5->at(probe)<0.1292 )
	 {
	   p=2;
	   passing_probe = passing_probe + 1;
	 }
   ...
   cout<<endl<<"Passingprobe : "<<passing_probe<<"  Failingprobe : "<<failing_probe<<endl<<endl;
