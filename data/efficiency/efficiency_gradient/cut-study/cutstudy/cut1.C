...
void cut1::Loop()
{
  ...
  for(int y=0; y<z; ++y)
    {
        ...
        if(nEle>1)
          {
            ...
            if(fabs(eleSCEta->at(probe))<1.479 )
              {      
		if(fabs(eledPhiAtVtx->at(probe))<0.0+0.001*y)
                  {
      ...
      Double_t fa = 0+0.001*y;//(failing_probe)/(passing_probe + failing_probe);
      ef[y]=effhisto;
      ef2[y]=fa;
  ...
  //  t->Print("eledPhiAtVtx_barrel_passing_by_passing+failing_vs_values.pdf");
