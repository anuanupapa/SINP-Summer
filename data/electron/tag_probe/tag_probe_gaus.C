#define tag_probe_gaus_cxx
#include "tag_probe_gaus.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void tag_probe_gaus::Loop()
{
    ...
    for(int k=0;  k<nEle; ++k)
      {
        ...
        if(fabs(eleSCEta->at(k))<1.479 && eleHoverE->at(k)<0.0414 && eleSigmaIEtaIEtaFull5x5->at(k)<0.00998 &&  eleMissHits->at(k)<=1 && eleConvVeto->at(k)==1 && elePt->at(k)>5 && eleCharge->at(tag)!=eleCharge->at(k))
          {
  ...
  */
  TF1 *func = new TF1("func","exp(((x-[1])*(x-[1]))/[2])",30,150);
  ...
  //  func->SetParameter(1,90.0);
  //  func->SetParameter(2,5.0);
  h->Fit("gaus");
  
  //func->Draw("Same");
  // c->Print("tag_probe_ele_pos.pdf");                                                                                                                                                                                                                                                                                                                              
}
/*
Double_t mybw(Double_t* x, Double_t* par)
{
